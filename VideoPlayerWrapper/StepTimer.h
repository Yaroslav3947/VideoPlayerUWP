...
class StepTimer {
 public:
  ...
  // operation) call this to avoid fixed timestep logic attempting a string of
  // catch-up Update calls.
  ...
  void Tick(const TUpdate& update) {
    ...
    // Convert QPC units into our own canonical tick format. Cannot overflow due
    ...
    if (m_isFixedTimeStep) {
      ...
      // If we are running very close to the target elapsed time (within 1/4 ms)
      // we just clamp the clock to exactly match our target value. This
      // prevents tiny and irrelvant errors from accumulating over time. Without
      // this clamping, a game that requested a 60 fps fixed update, running
      // with vsync enabled on a 59.94 NTSC display, would eventually accumulate
      // enough tiny errors that it would drop a frame. Better to just round
      // such small deviations down to zero, and leave things running smoothly.

      if (abs((int64)(timeDelta - m_targetElapsedTicks)) <
          TicksPerSecond / 4000) {
